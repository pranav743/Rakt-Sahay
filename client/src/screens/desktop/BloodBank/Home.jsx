...
import { Grid, GridItem, StatLabel, Stat, StatHelpText, StatNumber, SimpleGrid, Box } from '@chakra-ui/react';
...
function combineBloodData(hospitalData, bloodBankData) {
    ...
    const combinedArray = [
        ["A+", bloodBankMap.get("A+") || 0, hospitalData.find(({ group }) => group === "A+")?.units || 0],
        ["B+", bloodBankMap.get("B+") || 0, hospitalData.find(({ group }) => group === "B+")?.units || 0],
        ["A-", bloodBankMap.get("A-") || 0, hospitalData.find(({ group }) => group === "A-")?.units || 0],
        ["B-", bloodBankMap.get("B-") || 0, hospitalData.find(({ group }) => group === "B-")?.units || 0],
        ["AB+", bloodBankMap.get("AB+") || 0, hospitalData.find(({ group }) => group === "AB+")?.units || 0],
        ["AB-", bloodBankMap.get("AB-") || 0, hospitalData.find(({ group }) => group === "AB-")?.units || 0],
        ["O+", bloodBankMap.get("O+") || 0, hospitalData.find(({ group }) => group === "O+")?.units || 0],
        ["O-", bloodBankMap.get("O-") || 0, hospitalData.find(({ group }) => group === "O-")?.units || 0],
    ];
...
const BloodBankHome = () => {
    ...
    const getHospitals = async () => {
        ...
        const graphData = (combineBloodData(res.data.data[2].currentBlood, user.bloodInventory));
        setBarData(graphData);
    }

    const setHospitalOnGraph = (i) => {
        const graphData = combineBloodData(data[i].currentBlood, user.bloodInventory);
    ...
    return (<>
        ...
        >
            ...
            <Box m={3} mt={8}> <p style={{ fontSize: '22px' }}>Blood Storage Statistics</p></Box>
            <BarChart id="graph" data={barData} />
            <Box m={3} mt={8}> <p style={{ fontSize: '22px' }}>Inventories of Hospitals in, {user && user.city}</p></Box>
            {data && user && data.map((card, index) => (
                <Box
